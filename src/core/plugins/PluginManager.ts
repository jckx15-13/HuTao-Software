...
import { cacheLayer } from "../data/CacheLayer";
import { useStore } from "@/core/state/store";

import { resolveEngineUrl } from "../data/resolveEngineUrl";
import { fetchLocalEngineManifest } from "../data/engineManifest";
...
class PluginManager {
    ...
    async registerPlugin(plugin: WorldPlugin): Promise<void> {
        ...
        if (typeof import.meta.env !== "undefined") {
            for (const [key, value] of Object.entries(import.meta.env)) {
                if (key.startsWith("NEXT_PUBLIC_WWV_PLUGIN_")) {
                    envVars[key.replace("NEXT_PUBLIC_WWV_PLUGIN_", "")] = (value as string) || "";
                }
            }
        }
        ...
        const rawEngineUrl = import.meta.env.NEXT_PUBLIC_WWV_PLUGIN_DATA_ENGINE_URL;
        ...
        const edition = (import.meta.env.NEXT_PUBLIC_WWV_EDITION || "local") as "local" | "cloud" | "demo";
