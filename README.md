...
`npm test` includes an integration contract check that verifies the mapped WorldwideView assets, Odysseus assets/source modules, credential-engine contracts, connector-engine contracts, server-provider bridge/verifier contracts, redacted Bridge connector-provider contracts, and local bridge routes still exist.
