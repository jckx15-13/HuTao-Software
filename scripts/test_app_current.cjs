...
(async () => {
  // Save screenshots to the current brain artifacts directory to avoid triggering Vite's watcher
  const screenshotsDir = 'C:\\Users\\jaron\\.gemini\\antigravity\\brain\\603fb42f-a824-4818-9518-0252d22e9359\\test_screenshots';
  ...
  async function setupPage() {
    ...
    page.on('requestfailed', request => {
      ...
      if (!url.includes('google-analytics') && !url.includes('doubleclick') && !url.includes('favicon.ico') && !url.includes('satellites/25544') && !url.includes('chat') && !url.includes('manifest')) {
        // Only treat as error if not a standard fallback/local probe abort
  ...
  try {
    ...
    const openedSettings = await safeEvaluate(() => {
      // Look for settings button icon or header trigger
      const settingsBtn = document.querySelector('button[title="Open settings"]') || document.querySelector('button[title="Open Settings"]') || document.querySelector('button[aria-label="Close settings"]') || document.querySelector('button[aria-label="Open settings"]');
      if (settingsBtn) {
        settingsBtn.click();
        return true;
      }
    ...
    const closedSettings = await safeEvaluate(() => {
      const closeBtn = document.querySelector('button[title="Exit Settings"]') || document.querySelector('button[title="Close settings"]') || Array.from(document.querySelectorAll('button')).find(b => b.textContent && b.textContent.includes('Back to Workspace'));
      if (closeBtn) {
        closeBtn.click();
        return true;
      }
    ...
    const reportPath = 'C:\\Users\\jaron\\.gemini\\antigravity\\brain\\603fb42f-a824-4818-9518-0252d22e9359\\automated_test_report.json';
    const reportData = {
