...
- `bridge/server.py` owns server-side provider credential resolution for OpenAI-compatible model routes. It can use `OPENAI_API_KEY` + `OPENAI_MODEL`, `OPENROUTER_API_KEY` + `OPENROUTER_MODEL`, `MISTRAL_API_KEY` + `MISTRAL_MODEL`, `PERPLEXITY_API_KEY` + `PERPLEXITY_MODEL`, or `GROQ_API_KEY` + `GROQ_MODEL` without returning secret values to the browser. It also exposes redacted server-side connector readiness for OpenAI, Anthropic, OpenRouter, Mistral, Perplexity, Groq, Apify, Google Cloud, GitHub, Notion, and OpenWeather through `/api/credentials/providers` and `/api/connectors/providers`; `/api/connectors/providers?probe=true` and `/api/connectors/probe/{provider_id}` actively check configured Bridge environment credentials while returning only redacted status fields.
- `bridge/server.py` exposes `/api/integration/status` as the feature reality ledger. It classifies features as `verified`, `source-backed`, `partial`, `unconfigured`, or `missing`, reports all three repository integrations, and returns a below-100 score plus `not_100_reason` while external credentials or live provider checks remain absent.
...
- Current 2026-06-26 connector pass: Settings -> AI Configuration reads `/api/connectors/providers?probe=true`, displays per-provider probe state, and Bridge exposes `/api/connectors/probe/{provider_id}` for active redacted credential checks. Missing provider keys are reported as `missing_credentials`, not treated as working integrations.
- Current 2026-06-26 feature reality pass: Developer Settings reads `/api/integration/status` and displays the source-backed feature ledger for Silver Wolf VI, WorldWideView, Odysseus, chat, UI overlap, imagery, astronomy, cursor fallback, credential engine, connector probes, server AI provider routing, and runtime services. The ledger intentionally stays below 100 while live external credentials are absent.
- Current 2026-06-26 validation: `npm test`, `npm run lint`, `npm run build`, and `pnpm audit --audit-level=high` pass. The repository integration contract reports 96/100, not 100/100, because runtime dependency scoring keeps the missing real model/provider endpoint visible.
...
4. `scripts/test_integration_contracts.cjs` verifies that mapped WorldWideView assets, copied Odysseus documentation assets, Odysseus source-module references, credential-engine provider contracts, connector-engine contracts, server-provider bridge/verifier contracts, redacted Bridge connector-provider/probe contracts, feature-reality ledger contracts, bridge routes, and bridge CORS defaults still exist.
...
- `npm run build`
- `npm run perf:check`
- `npm run perf:check:fast`
- `npm run qa:check`
- `npm run test`
- `npm run typecheck`
- `npm run ui:audit`
- `npm run ui:audit:fast`
